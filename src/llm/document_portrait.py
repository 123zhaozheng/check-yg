...
- amount_sign_rule: 金额符号规则，必须为以下之一：
  - 正数表示收入: 正数=收入，负数=支出（如"收入/支出金额"列，正数对应存入/转入）
  - 正数表示支出: 正数=支出，负数=收入（如信用卡账单，正数对应消费/刷卡）
  - 无正负号按摘要判断: 金额无正负号，靠摘要/收支列判断
  - 收入支出分列: 收入/支出分两列（或借方/贷方双列）
  - 未知: 无法判断
...
- 若金额列全部为正数或绝对值，无负号 → 无正负号按摘要判断
- 若存在"收入金额"+"支出金额"或"借方金额"+"贷方金额"两列 → 收入支出分列
- 若正数行对应存入/转入/贷款发放等，负数行对应扣款/转出/还款等 → 正数表示收入
- 若正数行对应消费/刷卡/分期等，负数行对应还款/退款等（信用卡常见） → 正数表示支出
...
## 返回JSON格式
{
  ...
  "statement_period": "",
  "key_observations": [],
  "amount_sign_rule": "正数表示收入 | 正数表示支出 | 无正负号按摘要判断 | 收入支出分列 | 未知"
}
"""
